...
public class PointDistributor : MonoBehaviour
{
    ...
    // Use IEnumerator Start to allow yielding on coroutines
    IEnumerator Start()
    {
        if (mainCamera == null)
        {
        ...
        GeneratePoints();
        // Wait for CaptureViews to complete
        yield return StartCoroutine(CaptureViews());

        // After CaptureViews has completed, proceed with quitting
        #if UNITY_EDITOR
